...
all_outputs <- output_base  %>% 
  mutate(tot_profits = profit_per_t.t_1 + profit_per_t.t_2, 
         tot_stock = stock.s_1 + stock.s_2 + stock.s_3 + stock.s_4 + stock.s_5 + stock.s_6,
...
species1 <- "Yellow-eyed snapper"
species2 <- "Deepwater blackgin snapper"
species3 <- "Southern red snapper"
species4 <- "Queen snapper"
species5 <- "Vermillion snapper"
species6 <- "Misty grouper"

#species1 <- "Yellow-eyed snapper (Lutjanus vivanus)"
#species2 <- "Deep water blackgin snapper (Lutjanus buccanella)"
#species3 <- "Southern red snapper (Lutjanus purpureus)"
#species4 <- "Queen snapper (Etelis oculatus)"
#species5 <- "Vermillion snapper (Rhomboplites aurorubens)"
#species6 <- "Misty grouper (Hyporthodus mystacinus)"
...
#save files
msylist <- c("abio1", "abio2", "abio3", "abio4", "abio5", "abio6")
...
msylist <- c("rev1", "rev2", "rev3", "rev4", "rev5", "rev6")
...
# save graphs
msylist <- c("har1", "har2", "har3", "har4", "har5", "har6")
#msylist <- as.list(msylist)
...
# save graphs
msylist <- c("cpue1_1", "cpue1_2", "cpue1_3", "cpue1_4", "cpue1_5", "cpue1_6")
#msylist <- as.list(msylist)
...
bio_all <- all_outputs %>% 
  select(-Label, -Label1) %>% 
  mutate(bmsy_all = bmsy1p + bmsy2p + bmsy3p + bmsy4p + bmsy5p + bmsy6p)
...
bio_all <- bio_all %>%
  ...
  summarise(s1=sum(bmsy1p)/years,
            ...
            s5=sum(bmsy5p)/years,
            s6=sum(bmsy6p)/years,
            s7=sum(bmsy_all)/(years*6),
            .groups = 'drop') %>% 
  ungroup() %>% 
  filter(s1>0, s2>0, s3>0, s4>0, s5>0, s6>0)
...
# save graphs
msylist <- c("gcpue1_1", "gcpue1_2", "gcpue1_3", "gcpue1_4", "gcpue1_5", "gcpue1_6")
#msylist <- as.list(msylist)
...
pro <-profit_single %>%
  ...
  mutate(total = s1 + s2 + s3 + s4 + s5 + s6)
...
ggsave(plot = final_result2, filename = here(fileplace, "all_results", "figures2", "figure2_basket4.png"), height = 5, width = 8)
...
bio_all1 <- all_outputs %>% 
  select(-Label, -Label1) %>% 
  mutate(bmsy_all = tot_stock/(0.5*(k1+k2+k3+k4+k5+k6))) %>% 
  filter(year==30) %>% 
...
final_result3 <- ggplot(success_long3, aes(x = bio, y = total)) + # 
  ...
  geom_text(data = success_long3%>% filter(fraction > 66), aes(label = fraction, x = bio, y = total), size = 2) + #success_long2 
  labs(title=success_title,
       subtitle=subtitle1,
...
ggsave(plot = final_result3, filename = here(fileplace, "all_results", "figures2", "profit1", "figure2_basket4.png"), height = 5, width = 8)
