...
k1 <- 811.1242516 #Lutjanus jocu
k2 <- 755.3126043 #Lutjanus cyanopterus
BMSY1=0.5*k1
BMSY2=0.5*k2
...
species1 <- "Dog snapper"
species2 <- "Cubera snapper"

#species1 <- "dog snapper (Lutjanus jocu) "
#species2 <- "cubera snapper (Lutjanus cyanopteruss)"
...
abio2 <- ggplot()+
  ...
  geom_text(data = all_outputs%>% filter(order > 1 & order < 100), aes(label = Label, 
                                                                       x = year + 0.5, 
                                                                       y = stock.s_2), size = 2) +
  labs(title=title2,
       subtitle=subtitle1,
...
final_result <- ggplot(success_long, aes(x = bio, y = total)) + # 
  ...
  geom_text(data = success_long%>% filter(fraction > 98), aes(label = fraction, 
                                                              x = bio, 
                                                              y = total), size = 2) +
...
final_result2 <- ggplot(success_long2, aes(x = bio, y = rev)) + # 
  ...
  #geom_label(aes(label = fraction), size = 5) + 
  #geom_text(hjust=1.5, vjust=0, size = 5/.pt)+
  geom_text(data = success_long2%>% filter(fraction > 98), aes(label = fraction,
...
ggsave(plot = final_result2, filename = here(fileplace, "all_results", "figures2", "figure2_basket5.png"), height = 5, width = 8)
...
ggsave(plot = final_result3, filename = here(fileplace, "all_results", "figures2", "profit1", "figure2_basket5.png"), height = 5, width = 8)
