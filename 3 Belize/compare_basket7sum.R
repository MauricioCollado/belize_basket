...
all_outputs <- output_base  %>% 
  mutate(tot_profits = profit_per_t.t_1 + profit_per_t.t_2, 
         tot_stock = stock.s_1 + stock.s_2 + stock.s_3,
...
species1 <- "Snook"
species2 <- "Bay snook"
species3 <- "Black-eyed catfish"

#species1 <- "snook (Centropomus undecimalis) "
#species2 <- "bay snook (Petenia splendida)"
#species3 <- "black-eyed catfish (Ictalurus furcatus)"

title1 <- paste0("Stock"," ", species1)
title2 <- paste0("Stock"," ", species2)
title3 <- paste0("Stock"," ", species3)

subtitle1 <- "1 basket, 3 species, 1 gear type, MSY sum quota"
...
abio3 <- ggplot()+
  ...
  mytheme +
  theme(legend.position = "none") +
  geom_hline(yintercept=c(0.25*k2, 0.5*k3), linetype='dashed') +
...
# save graphs
msylist <- c("cpue1_1", "cpue1_2", "cpue1_3")
#msylist <- as.list(msylist)
...
bio_all <- all_outputs %>% 
  select(-Label, -Label1) %>% 
  mutate(bmsy_all = bmsy1p + bmsy2p + bmsy3p)
...
bio_all <- bio_all %>%
  ...
  summarise(s1=sum(bmsy1p)/years,
            ...
            s3=sum(bmsy3p)/years,
            #s4=sum(bmsy4p)/years,
            s5=sum(bmsy_all)/(years*3),
            .groups = 'drop') %>% 
  ungroup() %>% 
  filter(s1>0, s2>0, s3>0)
...
# save graphs
msylist <- c("gcpue1_1", "gcpue1_2", "gcpue1_3")
#msylist <- as.list(msylist)
...
final_result <- ggplot(success_long, aes(x = bio, y = total)) + # 
  ...
  geom_text(data = success_long%>% filter(fraction > 39), aes(label = fraction, 
                                                                       x = bio, 
                                                                       y = total), size = 2) +
...
final_result2 <- ggplot(success_long2, aes(x = bio, y = rev)) + # 
  ...
  #geom_label(aes(label = fraction), size = 5) + 
  #geom_text(hjust=1.5, vjust=0, size = 5/.pt)+
  geom_text(data = success_long2%>% filter(fraction > 39), aes(label = fraction,
...
ggsave(plot = final_result2, filename = here(fileplace, "all_results", "figures2", "figure2_basket7.png"), height = 5, width = 8)
...
bio_all1 <- all_outputs %>% 
  select(-Label, -Label1) %>% 
  mutate(bmsy_all = tot_stock/(0.5*(k1+k2+k3))) %>% 
  filter(year==30) %>% 
...
final_result3 <- ggplot(success_long3, aes(x = bio, y = total)) + # 
  ...
  geom_text(data = success_long3%>% filter(fraction > 44), aes(label = fraction, x = bio, y = total), size = 2) + #success_long2 
  labs(title=success_title,
       subtitle=subtitle1,
...
ggsave(plot = final_result3, filename = here(fileplace, "all_results", "figures2", "profit1", "figure2_basket7.png"), height = 5, width = 8)
