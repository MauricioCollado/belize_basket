###### Different progress for each limit
###### Code by Mauricio Collado

###### Compare progress for each harvest SUM limit
#####

# erase
rm(list = ls(all = TRUE)) 


# packages
library(tidyverse)
library(here)
library(scales)
library(gridExtra)
library(grid)
library(kableExtra)
library(ggtext)
library(ggplot2)
library(ggrepel)
library(metR)

# Where to save datasets
fileplace <- "3 Belize"
# Type of model, in this case we have the simple scenarios
fileplace1 <- "basket1"
fileplace2 <- "figures"
filesingle <- "basket1"

########################################################################
########################################################################
# read files

#directory 
base_dir <- here(fileplace, fileplace1, "results") 

# temp to list files
temp = list.files(path=base_dir, pattern="*.csv", full.names = TRUE)

# temp to list names
temp1 = list.files(path=base_dir, pattern="*.csv")

# read it all!
for (i in 1:length(temp)) assign(temp1[i], read.csv(temp[i]))

#bind each dataset

#test
df <- bind_rows(output_0.05_.csv, output_0.1_.csv, .id = "id")

# dataset
df <- bind_rows(mget(temp1), .id = "id") 

df_col <- df %>% 
  mutate(id = as.character(gsub("\\D", "", id)))
  #  mutate(id = gsub(".*[_]([^.]+)[.]_*", "\\1", id)) %>% 
  # mutate(id = substr(id, 1, 3))

########################################################################
########################################################################
# optimal path

mortguess1 <- c(0.00001, 0.00001, 0.00001, 0.00001, 0.00001, 24.23, 385.96, 561.46, 612, 
                612, 612, 612, 612, 612, 612, 612, 612, 612, 612,612, 612, 612, 612, 612, 
                612, 612, 612, 612, 612, 612)

year_mort <- seq(1,30, 1)

optim_path <- data.frame(year_mort, mortguess1) 

names(optim_path) <- c("year", "harvest.s_1")


########################################################################
########################################################################
# extra data
# MSY inputs

# additional data. 
# NOTE: this applies if every species hjave the same K


k1 <- 17.27910103	#	Coryphaena hippurus
k2 <- 822.3731754	#	Kajikia albida
k3 <- 914.4006364	#	Kajikia audax
k4 <- 935.7060958	#	Xiphias gladius
k5 <- 615.3019578	#	Thunnus albacares
k6 <- 238.3418413	#	Rachycentron canadum
k7 <- 81.62535576	#	Acanthocybium solandri
k8 <- 272.7010473	#	Seriola dumerili

BMSY1=0.5*k1
BMSY2=0.5*k2
BMSY3=0.5*k3
BMSY4=0.5*k4
BMSY5=0.5*k5
BMSY6=0.5*k6
BMSY7=0.5*k7
BMSY8=0.5*k8


msy_thresh=0.99999
years=30

########################################################################
########################################################################

# rep(c(1, 2, 3), times=10)
# seq(0,30)

output_base <- df_col %>% 
  mutate(year = rep(seq(0,30), times=100), # before times = 20
         bmsy1=stock.s_1/BMSY1,
         bmsy2=stock.s_2/BMSY2,
         bmsy3=stock.s_3/BMSY3,
         bmsy4=stock.s_4/BMSY4,
         bmsy5=stock.s_5/BMSY5,
         bmsy6=stock.s_6/BMSY6,
         bmsy7=stock.s_7/BMSY7,
         bmsy8=stock.s_8/BMSY8,
         bmsy1p=0,
         bmsy2p=0,
         bmsy3p=0,
         bmsy4p=0,
         bmsy5p=0,
         bmsy6p=0,
         bmsy7p=0,
         bmsy8p=0)


#Group them
all_outputs <- output_base  %>% 
  mutate(tot_profits = profit_per_t.t_1 + profit_per_t.t_2, 
         tot_stock = stock.s_1 + stock.s_2 + stock.s_3 + stock.s_4 + stock.s_5 + stock.s_6 + stock.s_7 + stock.s_8,
         cpue1_1 = harvest.s_1/effort.t_1,
         cpue2_1 = harvest.s_2/effort.t_1,
         cpue3_1 = harvest.s_3/effort.t_1,
         cpue4_1 = harvest.s_4/effort.t_1,
         cpue5_1 = harvest.s_5/effort.t_1,
         cpue6_1 = harvest.s_6/effort.t_1,
         cpue7_1 = harvest.s_7/effort.t_1,
         cpue8_1 = harvest.s_8/effort.t_1,
         gcpue1_1=(cpue1_1 - lag(cpue1_1))/lag(cpue1_1),
         gcpue2_1=(cpue2_1 - lag(cpue2_1))/lag(cpue2_1),
         gcpue3_1=(cpue3_1 - lag(cpue3_1))/lag(cpue3_1),
         gcpue4_1=(cpue4_1 - lag(cpue4_1))/lag(cpue4_1),
         gcpue5_1=(cpue5_1 - lag(cpue5_1))/lag(cpue5_1),
         gcpue6_1=(cpue6_1 - lag(cpue6_1))/lag(cpue6_1),
         gcpue7_1=(cpue7_1 - lag(cpue7_1))/lag(cpue7_1),
         gcpue8_1=(cpue8_1 - lag(cpue8_1))/lag(cpue8_1)) %>% 
  mutate(id2 = case_when(id == "001" ~ "1",
                         id == "002" ~ "2",
                         id == "003" ~ "3",
                         id == "004" ~ "4",
                         id == "005" ~ "5",
                         id == "006" ~ "6",
                         id == "007" ~ "7",
                         id == "008" ~ "8",
                         id == "009" ~ "9",
                         id == "01" ~ "10",
                         id == "011" ~ "11",
                         id == "012" ~ "12",
                         id == "013" ~ "13",
                         id == "014" ~ "14",
                         id == "015" ~ "15",
                         id == "016" ~ "16",
                         id == "017" ~ "17",
                         id == "018" ~ "18",
                         id == "019" ~ "19",
                         id == "02" ~ "20",
                         id == "021" ~ "21",
                         id == "022" ~ "22",
                         id == "023" ~ "23",
                         id == "024" ~ "24",
                         id == "025" ~ "25",
                         id == "026" ~ "26",
                         id == "027" ~ "27",
                         id == "028" ~ "28",
                         id == "029" ~ "29",
                         id == "03" ~ "30",
                         id == "031" ~ "31",
                         id == "032" ~ "32",
                         id == "033" ~ "33",
                         id == "034" ~ "34",
                         id == "035" ~ "35",
                         id == "036" ~ "36",
                         id == "037" ~ "37",
                         id == "038" ~ "38",
                         id == "039" ~ "39",
                         id == "04" ~ "40",
                         id == "041" ~ "41",
                         id == "042" ~ "42",
                         id == "043" ~ "43",
                         id == "044" ~ "44",
                         id == "045" ~ "45",
                         id == "046" ~ "46",
                         id == "047" ~ "47",
                         id == "048" ~ "48",
                         id == "049" ~ "49",
                         id == "05" ~ "50",
                         id == "051" ~ "51",
                         id == "052" ~ "52",
                         id == "053" ~ "53",
                         id == "054" ~ "54",
                         id == "055" ~ "55",
                         id == "056" ~ "56",
                         id == "057" ~ "57",
                         id == "058" ~ "58",
                         id == "059" ~ "59",
                         id == "06" ~ "60",
                         id == "061" ~ "61",
                         id == "062" ~ "62",
                         id == "063" ~ "63",
                         id == "064" ~ "64",
                         id == "065" ~ "65",
                         id == "066" ~ "66",
                         id == "067" ~ "67",
                         id == "068" ~ "68",
                         id == "069" ~ "69",
                         id == "07" ~ "70",
                         id == "071" ~ "71",
                         id == "072" ~ "72",
                         id == "073" ~ "73",
                         id == "074" ~ "74",
                         id == "075" ~ "75",
                         id == "076" ~ "76",
                         id == "077" ~ "77",
                         id == "078" ~ "78",
                         id == "079" ~ "79",
                         id == "08" ~ "80",
                         id == "081" ~ "81",
                         id == "082" ~ "82",
                         id == "083" ~ "83",
                         id == "084" ~ "84",
                         id == "085" ~ "85",
                         id == "086" ~ "86",
                         id == "087" ~ "87",
                         id == "088" ~ "88",
                         id == "089" ~ "89",
                         id == "09" ~ "90",
                         id == "091" ~ "91",
                         id == "092" ~ "92",
                         id == "093" ~ "93",
                         id == "094" ~ "94",
                         id == "095" ~ "95",
                         id == "096" ~ "96",
                         id == "097" ~ "97",
                         id == "098" ~ "98",
                         id == "099" ~ "99",
                         id == "1" ~ "100"),
         id2=ifelse(is.na(id2), id, id2),
         id = id2) %>% 
  select(-id2) %>% 
  mutate(Label = ifelse(year == 30, id, NA),
         Label1 = ifelse(year == 1, id, NA),
         order = as.numeric(as.character(id)))


# create msy indicator function
#create indicator of reaching MSY

all_outputs$bmsy1p<-ifelse(all_outputs$bmsy1>msy_thresh, 1, 0)
all_outputs$bmsy2p<-ifelse(all_outputs$bmsy2>msy_thresh, 1, 0)
all_outputs$bmsy3p<-ifelse(all_outputs$bmsy3>msy_thresh, 1, 0)
all_outputs$bmsy4p<-ifelse(all_outputs$bmsy4>msy_thresh, 1, 0)
all_outputs$bmsy5p<-ifelse(all_outputs$bmsy5>msy_thresh, 1, 0)
all_outputs$bmsy6p<-ifelse(all_outputs$bmsy6>msy_thresh, 1, 0)
all_outputs$bmsy7p<-ifelse(all_outputs$bmsy7>msy_thresh, 1, 0)
all_outputs$bmsy8p<-ifelse(all_outputs$bmsy8>msy_thresh, 1, 0)

########################################################################
########################################################################
# GRAPHS
########################################################################
########################################################################
### STOCK

species1 <- "Dolphinfish (Coryphaena hippurus)"
species2 <- "Marlin white (Kajikia albida)"
species3 <- "Marlin stripe (Kajikia audax)"
species4 <- "Swordfish (Xiphias gladius)"
species5 <- "Thunnus albacares (Yellowfin tuna)"
species6 <- "Cobia (Rachycentron canadum)"
species7 <- "Wahoo (Acanthocybium solandri)"
species8 <- "Great amberjack (Seriola dumerili)"

title1 <- paste0("Stock"," ", species1)
title2 <- paste0("Stock"," ", species2)
title3 <- paste0("Stock"," ", species3)
title4 <- paste0("Stock"," ", species4)
title5 <- paste0("Stock"," ", species5)
title6 <- paste0("Stock"," ", species6)
title7 <- paste0("Stock"," ", species7)
title8 <- paste0("Stock"," ", species8)

subtitle1 <- "1 basket, 6 species, 1 gear type, MSY sum quota"

mytheme<-theme( strip.background = element_rect(fill="white"),
                axis.ticks.length = unit(-0.05, "in"),
                axis.text.y = element_text(margin=unit(c(0.3,0.3,0.3,0.3), "cm")),
                axis.text.x = element_markdown(margin=unit(c(0.3,0.3,0.3,0.3), "cm")),
                legend.position = "bottom")

#mybreaks <- c(20,40,60,80)

abio1 <- ggplot()+
  #ggplot(data = all_outputs, aes(x=year, y=stock.s_1, color = id))+
  #geom_line() + 
  #geom_label(aes(label = Label), nudge_x = 0.35, size = 1) + 
  geom_line(data = all_outputs, aes(x=year, y=stock.s_1, color = id)) +
  geom_text(data = all_outputs%>% filter(order > 0 & order < 101), aes(label = Label, 
                                                               x = year + 0.5, 
                                                               y = stock.s_1), size = 2) +
  labs(title=title1,
       subtitle=subtitle1,
       y= "Stock",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none") +
  geom_hline(yintercept=c(0.25*k1, 0.5*k1), linetype='dashed') +
  annotate(geom="text", x=0, y=0.3*k1, label="25% K") +
  annotate(geom="text", x=0, y=0.55*k1, label="50% K")

abio1

#abio2 <- ggplot(data = all_outputs, aes(x=year, y=stock.s_2, color=id))+
 # geom_line()+
  # geom_label(aes(label = Label), nudge_x = 0.35, size = 1) + 
  #labs(title=title2,
   #    subtitle=subtitle1,
    #   y= "Stock",
     #  x= "Year")+
  #expand_limits(y = 0) +
  #theme_bw(base_size = 12) +
  #mytheme+
  #theme(legend.position = "none")+
  #geom_hline(yintercept=c(0.25*k2, 0.5*k2), linetype='dashed') +
  #annotate(geom="text", x=0, y=0.3*k2, label="25% K") +
  #annotate(geom="text", x=0, y=0.55*k2, label="50% K")

#  geom_hline(yintercept=c(0.25*k2, 0.5*k2), linetype='dashed', color=c('blue', 'red'))

abio2 <- ggplot()+
  #ggplot(data = all_outputs, aes(x=year, y=stock.s_1, color = id))+
  #geom_line() + 
  #geom_label(aes(label = Label), nudge_x = 0.35, size = 1) + 
  geom_line(data = all_outputs, aes(x=year, y=stock.s_2, color = id)) +
  geom_text(data = all_outputs%>% filter(order > 0 & order < 101), aes(label = Label, 
                                                                       x = year + 0.5, 
                                                                       y = stock.s_2), size = 2) +
  labs(title=title2,
       subtitle=subtitle1,
       y= "Stock",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none") +
  geom_hline(yintercept=c(0.25*k2, 0.5*k2), linetype='dashed') +
  annotate(geom="text", x=0, y=0.3*k2, label="25% K") +
  annotate(geom="text", x=0, y=0.55*k2, label="50% K")

abio2

abio3 <- ggplot()+
  #ggplot(data = all_outputs, aes(x=year, y=stock.s_1, color = id))+
  #geom_line() + 
  #geom_label(aes(label = Label), nudge_x = 0.35, size = 1) + 
  geom_line(data = all_outputs, aes(x=year, y=stock.s_3, color = id)) +
  geom_text(data = all_outputs%>% filter(order > 0 & order < 101), aes(label = Label, 
                                                                       x = year + 0.5, 
                                                                       y = stock.s_3), size = 2) +
  labs(title=title3,
       subtitle=subtitle1,
       y= "Stock",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none") +
  geom_hline(yintercept=c(0.25*k3, 0.5*k3), linetype='dashed') +
  annotate(geom="text", x=0, y=0.3*k3, label="25% K") +
  annotate(geom="text", x=0, y=0.55*k3, label="50% K")

abio3

abio4 <- ggplot()+
  #ggplot(data = all_outputs, aes(x=year, y=stock.s_1, color = id))+
  #geom_line() + 
  #geom_label(aes(label = Label), nudge_x = 0.35, size = 1) + 
  geom_line(data = all_outputs, aes(x=year, y=stock.s_4, color = id)) +
  geom_text(data = all_outputs%>% filter(order > 0 & order < 101), aes(label = Label, 
                                                                       x = year + 0.5, 
                                                                       y = stock.s_4), size = 2) +
  labs(title=title4,
       subtitle=subtitle1,
       y= "Stock",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none") +
  geom_hline(yintercept=c(0.25*k4, 0.5*k4), linetype='dashed') +
  annotate(geom="text", x=0, y=0.3*k4, label="25% K") +
  annotate(geom="text", x=0, y=0.55*k4, label="50% K")

abio4

abio5 <- ggplot()+
  #ggplot(data = all_outputs, aes(x=year, y=stock.s_1, color = id))+
  #geom_line() + 
  #geom_label(aes(label = Label), nudge_x = 0.35, size = 1) + 
  geom_line(data = all_outputs, aes(x=year, y=stock.s_5, color = id)) +
  geom_text(data = all_outputs%>% filter(order > 0 & order < 101), aes(label = Label, 
                                                                       x = year + 0.5, 
                                                                       y = stock.s_5), size = 2) +
  labs(title=title5,
       subtitle=subtitle1,
       y= "Stock",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none") +
  geom_hline(yintercept=c(0.25*k5, 0.5*k5), linetype='dashed') +
  annotate(geom="text", x=0, y=0.3*k5, label="25% K") +
  annotate(geom="text", x=0, y=0.55*k5, label="50% K")

abio5

abio6 <- ggplot()+
  #ggplot(data = all_outputs, aes(x=year, y=stock.s_1, color = id))+
  #geom_line() + 
  #geom_label(aes(label = Label), nudge_x = 0.35, size = 1) + 
  geom_line(data = all_outputs, aes(x=year, y=stock.s_6, color = id)) +
  geom_text(data = all_outputs%>% filter(order > 0 & order < 101), aes(label = Label, 
                                                                       x = year + 0.5, 
                                                                       y = stock.s_6), size = 2) +
  labs(title=title6,
       subtitle=subtitle1,
       y= "Stock",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none") +
  geom_hline(yintercept=c(0.25*k6, 0.5*k6), linetype='dashed') +
  annotate(geom="text", x=0, y=0.3*k6, label="25% K") +
  annotate(geom="text", x=0, y=0.55*k6, label="50% K")

abio6

abio7 <- ggplot()+
  #ggplot(data = all_outputs, aes(x=year, y=stock.s_1, color = id))+
  #geom_line() + 
  #geom_label(aes(label = Label), nudge_x = 0.35, size = 1) + 
  geom_line(data = all_outputs, aes(x=year, y=stock.s_7, color = id)) +
  geom_text(data = all_outputs%>% filter(order > 0 & order < 101), aes(label = Label, 
                                                                       x = year + 0.5, 
                                                                       y = stock.s_7), size = 2) +
  labs(title=title7,
       subtitle=subtitle1,
       y= "Stock",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none") +
  geom_hline(yintercept=c(0.25*k7, 0.5*k7), linetype='dashed') +
  annotate(geom="text", x=0, y=0.3*k7, label="25% K") +
  annotate(geom="text", x=0, y=0.55*k7, label="50% K")

abio7

abio8 <- ggplot()+
  #ggplot(data = all_outputs, aes(x=year, y=stock.s_1, color = id))+
  #geom_line() + 
  #geom_label(aes(label = Label), nudge_x = 0.35, size = 1) + 
  geom_line(data = all_outputs, aes(x=year, y=stock.s_8, color = id)) +
  geom_text(data = all_outputs%>% filter(order > 0 & order < 101), aes(label = Label, 
                                                                       x = year + 0.5, 
                                                                       y = stock.s_8), size = 2) +
  labs(title=title8,
       subtitle=subtitle1,
       y= "Stock",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none") +
  geom_hline(yintercept=c(0.25*k8, 0.5*k8), linetype='dashed') +
  annotate(geom="text", x=0, y=0.3*k8, label="25% K") +
  annotate(geom="text", x=0, y=0.55*k8, label="50% K")

abio8

#save files
msylist <- c("abio1", "abio2", "abio3", "abio4", "abio5", "abio6", "abio7", "abio8")

for(y in 1:length(msylist)){ 
  #filename = paste("msy_", y,".png", sep="")
  # plot1 <- msylist[[y]]
  ggsave(plot = get(msylist[y]), filename = here(fileplace, fileplace1, "figures", file=paste0("abio", y ,".png")), height = 5, width = 8)
}

########################################################################
########################################################################
### REVENUE

all_outputsr <- all_outputs  %>% 
  mutate(Label = ifelse(year == 20, id, NA))

title1rev <- paste0("Revenue (30y)"," ", species1)
title2rev <- paste0("Revenue (30y)"," ", species2)
title3rev <- paste0("Revenue (30y)"," ", species3)
title4rev <- paste0("Revenue (30y)"," ", species4)
title5rev <- paste0("Revenue (30y)"," ", species5)
title6rev <- paste0("Revenue (30y)"," ", species6)
title7rev <- paste0("Revenue (30y)"," ", species7)
title8rev <- paste0("Revenue (30y)"," ", species8)

rev1 <- ggplot(data = all_outputsr, aes(x=year, y=rev_per_sp.s_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label1), nudge_x = 0.35, size = 1)+ 
  labs(title=title1rev,
       subtitle=subtitle1,
       y= "Revenue $",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

rev1 

rev2 <- ggplot(data = all_outputsr, aes(x=year, y=rev_per_sp.s_2, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label1), nudge_x = 0.35, size = 1)+ 
  labs(title=title2rev,
       subtitle=subtitle1,
       y= "Revenue $",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

rev2 

rev3 <- ggplot(data = all_outputsr, aes(x=year, y=rev_per_sp.s_3, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label1), nudge_x = 0.35, size = 1)+ 
  labs(title=title3rev,
       subtitle=subtitle1,
       y= "Revenue $",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

rev3 

rev4 <- ggplot(data = all_outputsr, aes(x=year, y=rev_per_sp.s_4, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label1), nudge_x = 0.35, size = 1)+ 
  labs(title=title4rev,
       subtitle=subtitle1,
       y= "Revenue $",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

rev4 

rev5 <- ggplot(data = all_outputsr, aes(x=year, y=rev_per_sp.s_5, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label1), nudge_x = 0.35, size = 1)+ 
  labs(title=title5rev,
       subtitle=subtitle1,
       y= "Revenue $",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

rev5

rev6 <- ggplot(data = all_outputsr, aes(x=year, y=rev_per_sp.s_6, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label1), nudge_x = 0.35, size = 1)+ 
  labs(title=title6rev,
       subtitle=subtitle1,
       y= "Revenue $",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

rev6

rev7 <- ggplot(data = all_outputsr, aes(x=year, y=rev_per_sp.s_7, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label1), nudge_x = 0.35, size = 1)+ 
  labs(title=title7rev,
       subtitle=subtitle1,
       y= "Revenue $",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

rev7

rev8 <- ggplot(data = all_outputsr, aes(x=year, y=rev_per_sp.s_8, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label1), nudge_x = 0.35, size = 1)+ 
  labs(title=title8rev,
       subtitle=subtitle1,
       y= "Revenue $",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

rev8

# save graphs

msylist <- c("rev1", "rev2", "rev3", "rev4", "rev5", "rev6", "rev7", "rev8")
#msylist <- as.list(msylist)

for(y in 1:length(msylist)){ 
  #filename = paste("msy_", y,".png", sep="")
  # plot1 <- msylist[[y]]
  ggsave(plot = get(msylist[y]), filename = here(fileplace, fileplace1,"figures", file=paste0("rev", y ,".png")), height = 5, width = 8)
}

########################################################################
########################################################################
### HARVEST

title1har <- paste0("Harvest (30y)"," ", species1)
title2har <- paste0("Harvest (30y)"," ", species2)
title3har <- paste0("Harvest (30y)"," ", species3)
title4har <- paste0("Harvest (30y)"," ", species4)
title5har <- paste0("Harvest (30y)"," ", species5)
title6har <- paste0("Harvest (30y)"," ", species6)
title7har <- paste0("Harvest (30y)"," ", species7)
title8har <- paste0("Harvest (30y)"," ", species8)

har1 <- ggplot()+
  geom_line(data = all_outputsr, aes(x=year, y=harvest.s_1, color=id))+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  #### geom_point(data=optim_path, aes(x=year, y=harvest.s_1), size=1) +
  geom_label(data = all_outputsr, aes(x=year, y=harvest.s_1, label = Label1, alpha=0.1), nudge_x = 0.35, size = 1)+ 
  labs(title=title1har,
       subtitle=subtitle1,
       y= "Stock",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

har1 

har2 <- ggplot(data = all_outputsr, aes(x=year, y=harvest.s_2, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label1), nudge_x = 0.35, size = 1)+ 
  labs(title=title2har,
       subtitle=subtitle1,
       y= "Harvest",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

har2 

har3 <- ggplot(data = all_outputsr, aes(x=year, y=harvest.s_3, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label1), nudge_x = 0.35, size = 1)+ 
  labs(title=title3har,
       subtitle=subtitle1,
       y= "Harvest",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

har3

har4 <- ggplot(data = all_outputsr, aes(x=year, y=harvest.s_4, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label1), nudge_x = 0.35, size = 1)+ 
  labs(title=title4har,
       subtitle=subtitle1,
       y= "Harvest",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

har4 

har5 <- ggplot(data = all_outputsr, aes(x=year, y=harvest.s_5, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label1), nudge_x = 0.35, size = 1)+ 
  labs(title=title5har,
       subtitle=subtitle1,
       y= "Harvest",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

har5 

har6 <- ggplot(data = all_outputsr, aes(x=year, y=harvest.s_6, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label1), nudge_x = 0.35, size = 1)+ 
  labs(title=title6har,
       subtitle=subtitle1,
       y= "Harvest",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

har6 

har7 <- ggplot(data = all_outputsr, aes(x=year, y=harvest.s_7, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label1), nudge_x = 0.35, size = 1)+ 
  labs(title=title7har,
       subtitle=subtitle1,
       y= "Harvest",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

har7 

har8 <- ggplot(data = all_outputsr, aes(x=year, y=harvest.s_8, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label1), nudge_x = 0.35, size = 1)+ 
  labs(title=title8har,
       subtitle=subtitle1,
       y= "Harvest",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

har8 

# save graphs
msylist <- c("har1", "har2", "har3", "har4", "har5", "har6", "har7", "har8")
#msylist <- as.list(msylist)

for(y in 1:length(msylist)){ 
  #filename = paste("msy_", y,".png", sep="")
  # plot1 <- msylist[[y]]
  ggsave(plot = get(msylist[y]), filename = here(fileplace, fileplace1,"figures", file=paste0("har", y ,".png")), height = 5, width = 8)
}

########################################################################
########################################################################
### CPUE

title1ef <- "Effort (30y) on generic technology"
title2ef <- "Effort (30y) on NONE technology"

eff1 <- ggplot(data = all_outputs, aes(x=year, y=effort.t_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label1), nudge_x = 0.35, size = 1)+ 
  labs(title=title1ef,
       subtitle=subtitle1,
       y= "Effort",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

eff1 

eff2 <- ggplot(data = all_outputs, aes(x=year, y=effort.t_2, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label1), nudge_x = 0.35, size = 1)+ 
  labs(title=title2ef,
       subtitle=subtitle1,
       y= "Effort",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

eff2 

# save graphs
msylist <- c("eff1", "eff2")
#msylist <- as.list(msylist)

for(y in 1:length(msylist)){ 
  #filename = paste("msy_", y,".png", sep="")
  # plot1 <- msylist[[y]]
  ggsave(plot = get(msylist[y]), filename = here(fileplace, fileplace1,"figures", file=paste0("effort", y ,".png")), height = 5, width = 8)
}

########################################################################
########################################################################
### catch per unit of effort

title1cpue <- paste0("CPUE (30y)"," ", species1)
title2cpue <- paste0("CPUE (30y)"," ", species2)
title3cpue <- paste0("CPUE (30y)"," ", species3)
title4cpue <- paste0("CPUE (30y)"," ", species4)
title5cpue <- paste0("CPUE (30y)"," ", species5)
title6cpue <- paste0("CPUE (30y)"," ", species6)
title7cpue <- paste0("CPUE (30y)"," ", species7)
title8cpue <- paste0("CPUE (30y)"," ", species8)

cpue1_1 <- ggplot(data = all_outputs, aes(x=year, y=cpue1_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label), nudge_x = 0.35, size = 2)+ 
  labs(title=title1cpue,
       subtitle=subtitle1,
       y= "CPUE",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

cpue1_1

cpue1_2 <- ggplot(data = all_outputs, aes(x=year, y=cpue2_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label), nudge_x = 0.35, size = 2)+ 
  labs(title=title2cpue,
       subtitle=subtitle1,
       y= "CPUE",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

cpue1_2

cpue1_3 <- ggplot(data = all_outputs, aes(x=year, y=cpue3_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label), nudge_x = 0.35, size = 2)+ 
  labs(title=title3cpue,
       subtitle=subtitle1,
       y= "CPUE",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

cpue1_3

cpue1_4 <- ggplot(data = all_outputs, aes(x=year, y=cpue4_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label), nudge_x = 0.35, size = 2)+ 
  labs(title=title4cpue,
       subtitle=subtitle1,
       y= "CPUE",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

cpue1_4

cpue1_5 <- ggplot(data = all_outputs, aes(x=year, y=cpue5_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label), nudge_x = 0.35, size = 2)+ 
  labs(title=title5cpue,
       subtitle=subtitle1,
       y= "CPUE",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

cpue1_5

cpue1_6 <- ggplot(data = all_outputs, aes(x=year, y=cpue6_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label), nudge_x = 0.35, size = 2)+ 
  labs(title=title6cpue,
       subtitle=subtitle1,
       y= "CPUE",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

cpue1_6

cpue1_7 <- ggplot(data = all_outputs, aes(x=year, y=cpue7_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label), nudge_x = 0.35, size = 2)+ 
  labs(title=title7cpue,
       subtitle=subtitle1,
       y= "CPUE",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

cpue1_7

cpue1_8 <- ggplot(data = all_outputs, aes(x=year, y=cpue8_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label), nudge_x = 0.35, size = 2)+ 
  labs(title=title8cpue,
       subtitle=subtitle1,
       y= "CPUE",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

cpue1_8

# save graphs
msylist <- c("cpue1_1", "cpue1_2", "cpue1_3", "cpue1_4", "cpue1_5", "cpue1_6", "cpue1_7", "cpue1_8")
#msylist <- as.list(msylist)

for(y in 1:length(msylist)){ 
  #filename = paste("msy_", y,".png", sep="")
  # plot1 <- msylist[[y]]
  ggsave(plot = get(msylist[y]), filename = here(fileplace, fileplace1,"figures", file=paste0("cpue", y ,".png")), height = 5, width = 8)
}

########################################################################
########################################################################
### biomass indicator

bio_all <- all_outputs %>% 
  select(-Label, -Label1) %>% 
  mutate(bmsy_all = bmsy1p + bmsy2p + bmsy3p + bmsy4p + bmsy5p + bmsy6p + bmsy7p + bmsy8p)

# bio_all <- drop_na(bio_all)

bio_all <- bio_all %>%
  group_by(id) %>%
  select(starts_with("bmsy")) %>% 
  summarise(s1=sum(bmsy1p)/years,
            s2=sum(bmsy2p)/years,
            s3=sum(bmsy3p)/years,
            s4=sum(bmsy4p)/years,
            s5=sum(bmsy5p)/years,
            s6=sum(bmsy6p)/years,
            s7=sum(bmsy7p)/years,
            s8=sum(bmsy8p)/years,
            s9=sum(bmsy_all)/(years*8),
            .groups = 'drop') %>% 
  ungroup() %>% 
  filter(s1>0, s2>0, s3>0, s4>0, s5>0, s6>0, s7>0, s8>0)


write.table(na.omit(bio_all), here(fileplace, fileplace1,"tables", "basket.csv"),
            row.names=FALSE, sep=",")

########################################################################
########################################################################
### cpue growth

title1cpue <- paste0("Growth CPUE (30y)"," ", species1)
title2cpue <- paste0("Growth CPUE (30y)"," ", species2)
title3cpue <- paste0("Growth CPUE (30y)"," ", species3)
title4cpue <- paste0("Growth CPUE (30y)"," ", species4)
title5cpue <- paste0("Growth CPUE (30y)"," ", species5)
title6cpue <- paste0("Growth CPUE (30y)"," ", species6)
title7cpue <- paste0("Growth CPUE (30y)"," ", species7)
title8cpue <- paste0("Growth CPUE (30y)"," ", species8)

gcpue1_1 <- ggplot(data = all_outputs, aes(x=year, y=gcpue1_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label), nudge_x = 0.35, size = 2)+ 
  labs(title=title1cpue,
       subtitle=subtitle1,
       y= "Growth CPUE",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

gcpue1_1

gcpue1_2 <- ggplot(data = all_outputs, aes(x=year, y=gcpue2_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label), nudge_x = 0.35, size = 2)+ 
  labs(title=title2cpue,
       subtitle=subtitle1,
       y= "Growth CPUE",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

gcpue1_2

gcpue1_3 <- ggplot(data = all_outputs, aes(x=year, y=gcpue3_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label), nudge_x = 0.35, size = 2)+ 
  labs(title=title3cpue,
       subtitle=subtitle1,
       y= "Growth CPUE",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

gcpue1_3

gcpue1_4 <- ggplot(data = all_outputs, aes(x=year, y=gcpue4_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label), nudge_x = 0.35, size = 2)+ 
  labs(title=title4cpue,
       subtitle=subtitle1,
       y= "Growth CPUE",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

gcpue1_4

gcpue1_5 <- ggplot(data = all_outputs, aes(x=year, y=gcpue5_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label), nudge_x = 0.35, size = 2)+ 
  labs(title=title5cpue,
       subtitle=subtitle1,
       y= "Growth CPUE",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

gcpue1_5

gcpue1_6 <- ggplot(data = all_outputs, aes(x=year, y=gcpue6_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label), nudge_x = 0.35, size = 2)+ 
  labs(title=title6cpue,
       subtitle=subtitle1,
       y= "Growth CPUE",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

gcpue1_6

gcpue1_7 <- ggplot(data = all_outputs, aes(x=year, y=gcpue7_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label), nudge_x = 0.35, size = 2)+ 
  labs(title=title7cpue,
       subtitle=subtitle1,
       y= "Growth CPUE",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

gcpue1_7

gcpue1_8 <- ggplot(data = all_outputs, aes(x=year, y=gcpue8_1, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
  #geom_line(aes(y=stock.s_5), color = "purple", size=1)+
  #geom_line(aes(y=stock.s_6), color = "black", size=1)+
  #geom_line(aes(y=stock.s_3), color = "black", size=1)+
  geom_label(aes(label = Label), nudge_x = 0.35, size = 2)+ 
  labs(title=title8cpue,
       subtitle=subtitle1,
       y= "Growth CPUE",
       x= "Year")+
  expand_limits(y = 0) +
  theme_bw(base_size = 12) +
  mytheme +
  theme(legend.position = "none")

gcpue1_8

# save graphs
msylist <- c("gcpue1_1", "gcpue1_2", "gcpue1_3", "gcpue1_4", "gcpue1_5", "gcpue1_6", "gcpue1_7", "gcpue1_8")
#msylist <- as.list(msylist)

for(y in 1:length(msylist)){ 
  #filename = paste("msy_", y,".png", sep="")
  # plot1 <- msylist[[y]]
  ggsave(plot = get(msylist[y]), filename = here(fileplace, fileplace1, "figures", file=paste0("gcpue", y ,".png")), height = 5, width = 8)
}

######## revenue

npv <-  read_csv(here("2 single_sp", filesingle, "results", "npvcase.csv"))

single <- npv[ , grepl( "rev_per" , names(npv))] %>% 
  mutate(year = seq(0,30))

single <- drop_na(single)

rev_single <- single %>%
  summarise(s1=sum((rev_per_sp.s_1)),
            s2=sum((rev_per_sp.s_2)),
            s3=sum((rev_per_sp.s_3)),
            s4=sum((rev_per_sp.s_4)),
            s5=sum((rev_per_sp.s_5)),
            s6=sum((rev_per_sp.s_6)),
            s7=sum((rev_per_sp.s_7)),
            s8=sum((rev_per_sp.s_8))
  ) 

all_rev <- all_outputs

all_rev  <- all_rev %>% 
  select(id, year, starts_with("rev_per"))

all_rev  <- drop_na(all_rev)

all_rev1 <- all_rev  %>%
  group_by(id) %>%
  summarise(s1=sum((rev_per_sp.s_1)),
            s2=sum((rev_per_sp.s_2)),
            s3=sum((rev_per_sp.s_3)),
            s4=sum((rev_per_sp.s_4)),
            s5=sum((rev_per_sp.s_5)),
            s6=sum((rev_per_sp.s_6)),
            s7=sum((rev_per_sp.s_7)),
            s8=sum((rev_per_sp.s_8)),
            .groups = 'drop'
  )  



all_rev1$s1 <- all_rev1$s1/rev_single$s1
all_rev1$s2 <- all_rev1$s2/rev_single$s2
all_rev1$s3 <- all_rev1$s3/rev_single$s3
all_rev1$s4 <- all_rev1$s4/rev_single$s4
all_rev1$s5 <- all_rev1$s5/rev_single$s5
all_rev1$s6 <- all_rev1$s6/rev_single$s6
all_rev1$s7 <- all_rev1$s7/rev_single$s7
all_rev1$s8 <- all_rev1$s8/rev_single$s8

######## profit

profit_single <- npv %>% 
  select(starts_with("profit_per")) %>% 
  mutate(year = seq(0,30))

profit_single <- drop_na(profit_single)

pro <-profit_single %>%
  summarise(s1=sum((profit_per_t.t_1)),
            s2=sum((profit_per_t.t_2)),
            s3=sum((profit_per_t.t_3)),
            s4=sum((profit_per_t.t_4)),
            s5=sum((profit_per_t.t_5)),
            s6=sum((profit_per_t.t_6)),
            s7=sum((profit_per_t.t_7)),
            s8=sum((profit_per_t.t_8))
  ) %>% 
  mutate(total = s1 + s2 + s3 + s4 + s5 + s6 + s7 +s8)

all_profits  <- all_outputs %>% 
  select(id, year, starts_with("profit_per"))


all_profits    <- drop_na(all_profits  )

all_profits1  <- all_profits    %>%
  group_by(id) %>%
  summarise(s1=sum((profit_per_t.t_1)),
            s2=sum((profit_per_t.t_2)),
            .groups = 'drop'
  )  %>% 
  mutate(total = s1 + s2)


all_profits1$total <- all_profits1$total/pro$total

all_profits1 <- all_profits1 %>% 
  select(id, total)

write.table(na.omit(all_profits1), here(fileplace, fileplace1,"tables", "profits.csv"),
            row.names=FALSE, sep=",")


###### join
success <- left_join(bio_all, all_profits1, by="id") %>%  
  mutate(fraction = as.numeric(as.character(id)))

write.table(na.omit(success), here(fileplace, fileplace1,"tables", "combined_result.csv"),
            row.names=FALSE, sep=",")

success_long <- success %>% 
  pivot_longer(
    cols = starts_with("s"),
    names_to = "species",
    names_prefix = "s",
    values_to = "bio",
    values_drop_na = TRUE
  ) %>% 
  mutate(species = case_when(species == 1 ~ species1,
                             species == 2 ~ species2,
                             species == 3 ~ species3,
                             species == 4 ~ species4,
                             species == 5 ~ species5,
                             species == 6 ~ species6,
                             species == 7 ~ species7,
                             species == 8 ~ species8,
                             species == 9 ~ "All species")
  )


success_title <- "Trade-off of policy goals"

final_result <- ggplot(success_long, aes(x = bio, y = total)) + # 
  geom_point(aes(color=species, size=fraction), alpha=0.2)+
  #geom_label(aes(label = fraction), size = 5) + 
  #geom_text(hjust=1.5, vjust=0, size = 5/.pt)+
  geom_text(data = success_long%>% filter(fraction > 40), aes(label = fraction, 
                                                                       x = bio, 
                                                                       y = total), size = 2) +
  labs(title=success_title,
       subtitle=subtitle1,
       y= "Profit ratio",
       x= "% of succcesful conservation years")+
  expand_limits(y = 0) +
  theme_bw(base_size = 10) +
  mytheme+
  theme(
    legend.justification = 'left', 
    legend.position = 'bottom', legend.box = 'vertical', 
    legend.box.just = 'left')

final_result

ggsave(plot = final_result, filename = here(fileplace, fileplace1,"figures", "final_graph.png"), height = 5, width = 8)
