...
all_outputs <- output_base  %>% 
  mutate(tot_profits = profit_per_t.t_1 + profit_per_t.t_2, 
         tot_stock = stock.s_1 + stock.s_2 + stock.s_3 + stock.s_4,
...
title1 <- "Stock Black Grouper (Mycteroperca bonaci) "
title2 <- "Stock Goliath Grouper (Epinephelus itajara)"
title3 <- "Stock Tiger Grouper (Mycteroperca tigris)"
title4 <- "Stock Yellowfin Grouper (Mycteroperca venenosa)"
...
subtitle1 <- "1 basket, 4 species, 1 gear type, MSY sum quota"
...
abio4 <- ggplot(data = all_outputs, aes(x=year, y=stock.s_4, color=id))+
  geom_line()+
  #geom_line(aes(y=stock.s_2), color = "blue", size=1)+
  #geom_line(aes(y=stock.s_3), color = "green", size=1)+
  #geom_line(aes(y=stock.s_4), color = "orange", size=1)+
...
#save files
msylist <- c("abio1", "abio2", "abio3", "abio4")
...
title1rev <- "Revenue (30y) Black Grouper"
title2rev <- "Revenue (30y) Goliath Grouper"
title3rev <- "Revenue (30y) Tiger Grouper"
title4rev <- "Revenue (30y) Yellowfin Grouper"
...
title1cpue <- "CPUE (30y) of generic technology on Black Grouper"
title2cpue <- "CPUE (30y) of generic technology on Goliath Grouper"
title3cpue <- "CPUE (30y) of generic technology on Tiger Grouper"
title4cpue <- "CPUE (30y) of generic technology on Yellowfin Grouper"
...
# save graphs
msylist <- c("cpue1_1", "cpue1_2", "cpue1_3", "cpue1_4")
#msylist <- as.list(msylist)
...
title1cpue <- "Growth of CPUE (30y) of generic technology on Black Grouper"
title2cpue <- "Growth of CPUE (30y) of generic technology on Goliath Grouper"
title3cpue <- "Growth of CPUE (30y) of generic technology on Tiger Grouper"
title4cpue <- "Growth ofCPUE (30y) of generic technology on Yellowfin Grouper"
...
# save graphs
msylist <- c("gcpue1_1", "gcpue1_2", "gcpue1_3", "gcpue1_4")
#msylist <- as.list(msylist)
...
success_long <- success %>% 
  ...
  mutate(species = case_when(species == 1 ~ "Black grouper",
...
final_result <- ggplot(success_long, aes(x = bio, y = total)) + # 
  ...
  geom_text(data = success_long%>% filter(fraction > 9), aes(label = fraction, 
                                                                       x = bio, 
                                                                       y = total), size = 2) +
...
species1 <-  "Black grouper"
species2 <-  "Goliath grouper"
species3 <-  "Tiger grouper"
species4 <-  "Yellowfin grouper"
...
final_result2 <- ggplot(success_long2, aes(x = bio, y = rev)) + # 
  ...
  #geom_label(aes(label = fraction), size = 5) + 
  #geom_text(hjust=1.5, vjust=0, size = 5/.pt)+
  geom_text(data = success_long2%>% filter(fraction > 9), aes(label = fraction,
...
ggsave(plot = final_result2, filename = here(fileplace, "all_results", "figures2", "figure2_basket9.png"), height = 5, width = 8)
...
bio_all1 <- all_outputs %>% 
  select(-Label, -Label1) %>% 
  mutate(bmsy_all = tot_stock/(0.5*(k1+k2+k3+k4))) %>% 
  filter(year==30) %>% 
...
final_result3 <- ggplot(success_long3, aes(x = bio, y = total)) + # 
  ...
  geom_text(data = success_long3%>% filter(fraction > 76), aes(label = fraction, x = bio, y = total), size = 2) + #success_long2 
  labs(title=success_title,
       subtitle=subtitle1,
...
ggsave(plot = final_result3, filename = here(fileplace, "all_results", "figures2", "profit1", "figure2_basket.png"), height = 5, width = 8)
...
final_result3 <- ggplot(success_long3, aes(x = bio, y = total)) + # 
  ...
  #geom_text(hjust=1.5, vjust=0, size = 5/.pt)+
  geom_vline(xintercept = 0.99)+
  geom_text(data = success_long3%>% filter(fraction > 35), aes(label = fraction, x = bio, y = total), size = 2) + #success_long2 
  labs(title=success_title,
       subtitle=subtitle1,
       y= "Accumulated profit ratio",
       x= "Stock/Bmsy on year 30")+
...
ggsave(plot = final_result3, filename = here(fileplace, "all_results", "figures2", "profit1", "figure2_basket9.png"), height = 5, width = 8)
