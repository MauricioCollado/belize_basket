qb_stock <- function(species, tech, cost, baskets, mortality, years){
  ...
  # keep X+ G(X)
  stock_df2 <- data.frame(t(sp_df$X))
  ...
  for (i in 1:years){
    ...
    #  calculate X + G(X)
    diag_b <- tail(stock_df,1) + (sp_df$r)*exp(-sp_df$tcoef*temp_anom[i])*tail(stock_df,1)*(1-tail(stock_df,1)/sp_df$K)
    # Set biological extraction limit. We dont know the stock, but we cannot harvest negatives
    h1 <- c(pmin(h, diag_b))
    
    # Calculate stock growth with X(t+1) = Xt + Xt*r*(1-Xt/K) - h
    stock_next <- tail(stock_df,1) + (sp_df$r)*exp(-sp_df$tcoef*temp_anom[i])*tail(stock_df,1)*(1-tail(stock_df,1)/sp_df$K) - h1
  ...
  sp_names1 <- paste("sg", sp_df$s, sep = "_")
  ...
  # Return output
  return(list(stock = stock_df, stock_g = stock_df2, harvest = h_df, effort = e_df, profit_per_t = pft_df, rev_per_sp= rev_sp_df))
}